...
Для <ins>первого</ins> запуска приложения выполнить последовательно команды:
     ...
     - ```mvn compile```
     - ```mvn exec:java -Dexec.mainClass="ru.job4j.accidents.Job4jAccidentsApplication"```
<br><br> 
...
6. Вставить из буфера обмена url в адресную строку браузера:<br>
...
8. Документацию к проекту можно посмотреть в директории проекта в файле:<br>
